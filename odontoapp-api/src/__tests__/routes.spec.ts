...
describe('/users', () => {
  ...
  beforeAll(async () => {
    ...
    testDBAdmin = await prisma.users.create({
      data: {
        name: 'Admin user',
  ...
  beforeEach(async () => {
    ...
    const { body } = await request(myAppInstance)
      .post('/session')
      .send({ email: testDBUser.email, password: acceptableUserData.password });

    authenticatedUser = body.data;
  });
